...
def update_user(id: str, user: UpdateUser):
    stored_item_data = conn.user.find_one({"UserId": id})
    stored_item_model = UpdateUser(**stored_item_data)
    update_data = user.dict(exclude_unset=True)
    updated_item = stored_item_model.copy(update=update_data)
    conn.user.find_one_and_update({"UserId": id}, {
...
def delete_user(id: str):
    conn.user.find_one_and_delete({"UserId": id})
