...
def create_vacancy(vacancy: Vacancy):
    ...
    id = conn.vacancy.insert_one(jsonable_encoder(vacancyResponse)).inserted_id
    vacancy = conn.vacancy.find_one({"_id": id})
...
def delete_vacancy(id: str):
    conn.vacancy.find_one_and_delete({"VacancyId": id})
